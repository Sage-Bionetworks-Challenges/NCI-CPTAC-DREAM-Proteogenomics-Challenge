...
trainEN <- function(train_feat, train_obs, xTrain_feat, xTrain_obs) {
  fit <- glmnet(train_feat, train_obs) ## betas and lambdas are computed from the training set
  ...
  for (i in 1:ncol(fit$beta)) {
    xTrain_cIDX <- c(xTrain_cIDX, cIDX(xTrain_pred[,i], xTrain_obs))  ## Harrell???s c-index or concordance C
