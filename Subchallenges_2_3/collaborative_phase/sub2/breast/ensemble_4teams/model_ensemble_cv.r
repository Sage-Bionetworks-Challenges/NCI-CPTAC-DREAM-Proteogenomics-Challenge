...
p1=read.delim("cv_brca_hyyf.tsv",row.names=1,header=T)[,1:5]
p1=p1[rownames(p1)!="VGF",]
p2=read.csv("cv_brca_hyu.csv",row.names=1)[,1:5]
p3=read.delim("cv_brca_sunkyu.tsv",row.names=1,header=T)[,1:5]
p4=read.csv("cv_brca_deargen.csv",row.names=1)[,1:5]
...
w1=mean(apply(p1,2,mean,na.rm=T)) #0.4107833
w2=mean(apply(p2,2,mean,na.rm=T)) #0.4165995
w3=mean(apply(p3,2,mean,na.rm=T)) #0.4170335
w4=mean(apply(p4,2,mean,na.rm=T)) #0.5492728
