...
path1 <- paste0(path,"/DREAM_CPTAC/PROT_PREDICTION/DATA_RESULT_STORAGE/scores/")
pred_HGSC_PROT_by_RNA <- read.delim(paste0(path1,"sub2_cor.txt"), header=FALSE, row.names = 1) ; colnames(pred_HGSC_PROT_by_RNA) <- "mRNA"

# nice_boxplot (pred_HGSC_PROT_by_RNA, title=paste("Prediction for ",as.character(length(rownames(pred_HGSC_PROT_by_RNA)))," proteins",sep="" ), text_size= 25, title_size= 1.2)
# plot( rowMeans(prot, na.rm = T) , rowSums(is.na(prot))) 
```
